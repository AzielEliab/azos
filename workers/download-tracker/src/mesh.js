...
export const MESH_PRODUCT = "azos";
...
/** Hub cite / Worker mesh cross-map only. Local qnsd lives in qnm-node. Not a public proxy. */
export const QNS_CD = Object.freeze({
  ...
  designs: Object.freeze({
    qnm: AZIEL_RUNTIME_REPO + "/blob/main/docs/designs/QNM-WP-1.0.md",
    ...
    skill: AZIEL_RUNTIME_REPO + "/blob/main/SKILL.md",
  }),
  note: "QNS-CD-1.0 photon QNS1 packet transfer. Local qnsd is coded in qnm-node. Runtime cites + catalog field live in aziel-runtime. AZInterface holds pair custody. Hub cite / Worker mesh cross-map only — not a Softwares-tab product. No public qnsd proxy. No Node Gate. Author: Aziel Eliab only.",
  author: IDENTITY,
  identity: IDENTITY,
});
...
export function withQnsCd(data) {
...
export function meshPointer() {
  return withQnsCd({
    ...
    origin: RUNTIME + MESH_PATH,
    note: "PROXY to aziel-runtime /v1/mesh/* via AZIEL_RUNTIME. Not a local op. Not AnonBroadcast. Not AZMail's product-local ring. Not a public qnsd proxy. AZ-OS remains an ethics-coded remote shell. Full node process is local qnm-node/. QNS-CD-1.0 photon QNS1 packet transfer is a hub cite / Worker mesh cross-map only. " + MESH_NOTE,
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "azos_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
...
async function originFetch(env, pathAndQuery, init, request) {
  ...
  headers.set("X-Aziel-Runtime-Via", "azos-download-tracker");
